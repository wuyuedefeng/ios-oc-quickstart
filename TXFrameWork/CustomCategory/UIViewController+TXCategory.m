...
{
    [self tx_SetLeftBarButtonWithImage:image highImage:nil target:target action:action];
}
/**
 *  设置导航栏左侧按钮
 *
...
{
    [self tx_SetRightBarButtonWithImage:image highImage:nil target:target action:action];
}
/**
 *  设置导航栏右侧按钮
 *
 ...
 */
- (void)tx_SetTitleView:(UIView *)tiltleView
{
...
{
    [self tx_SetBackButtonWithWithImage:image highImage:nil target:target action:action];
}
/**
 *  设置导航栏push到另一个导航栏的返回按钮
 *
...
- (void)tx_setTabBarButtonClickColor:(UIColor *)color
{
...
{
    [self tx_loadAnimation_MBProgressHUD:block finishDid:nil];
}
